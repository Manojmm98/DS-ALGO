...
public class Main {
  ...
  System.out.println();
    ...
    void push(int val) {
      ...
      // if we want to create a stack with dynamic memory then we have to create a stack with double size and then we have to push elements one by one into it 
      // this situation occurs when the the size of data array is equal to the length of data array 
      
        if (size() == data.length ) {
        //System.out.println("Stack overflow");
        
        // cereate a new storage with double length
        
        int newData[]= new int[data.length*2];
        
        // copy the ements from old data array
        
        for(int i =0;i<data.length;i++){
            newData[i]=data[i];
        }
        
        // change the reference of data to newdata
